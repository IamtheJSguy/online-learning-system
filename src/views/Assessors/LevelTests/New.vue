  ...
  <div id="container" style="color: white; text-align: left; padding: 30px">
    ...
    <div class="row mt-4">
      <div class="col-lg-4 col-md-12 col-sm-12 sm-cards-background mt-2">
        ...
        <assessor-courses-card
            v-for="course in newCourses"
            :key="course._id"
            :course="course"
            @claimedCourse="getCourses"
        />
        <!--  -->
      </div>
...
<script>
import AssessorCoursesCard from '@/components/Assessors/Cards.vue'
export default {
  name: 'MyLevelTest',
  components: {
      AssessorCoursesCard
  ...
  methods: {
      getCourses() {
          this.$store.dispatch('candidateCoursesByAssessors')
            .then(
                (response) => {
                    this.newCourses = response.data.data.newCourses
                    this.reviewCourses = response.data.data.reviewCourses
                    this.doneCourses = response.data.data.doneCourses
                }
            )
      }
  }
